...
function MasteryChooser:update()
	...
	local scriptadress = "/S1mpleScripts/Scripts/BolStudio/Libary/MasteryChooser/"
...
function MasteryChooser:downloadSprite(spritename)
	...
	self.visible = false
	local serveradress = "scarjit.de"
	local scriptadress = "/S1mpleScripts/Scripts/BolStudio/Libary/MasteryChooser/Sprites/"
...
function MasteryChooser:downloadMastery()
	print("Downloading Mastery Data")
	GetMasteries()
end


class "GetMasteries" --by Aroc
function GetMasteries:__init()
	self.LuaSocket = require("socket")
	self.MasterySocket = self.LuaSocket.connect("www.scarjit", 80)
	self.MasterySocket:send("GET /S1mpleScripts/riotapi/getSummonerID.php?SID="..Zhokhizh.." HTTP/1.0\r\n\r\n")
	--local ret = self.MasterySocket:receive('*a')
	--print(ret)
end
