...
local function modPow(base,exp,modulus)
	if(eq(modulus, bigint(1)))then return bigint(0) end

	local result = bigint(1)
	local base = mod(base, modulus)
	while gt(exponent, bigint(0)) do
		local m2 = mod(exp, bigint(2))
		if(eq(m2,bigint(1)))then
			result = mod(multiply(result, base), modulus)
		end
		exponent = exponent >> 1 --TODO
...
function MessageToBin(m, keylenght)
	m = "1337"..m
	local mr = ""
	for i=1,m:len() do
		local mc = tostring(m:sub(i,i):byte())
		for i=1,4-mc:len() do
			mc = "0"..mc
		end
		mr = mr .. mc
	end
	for i=1,keylenght-mr:len()%keylenght+2 do
		mr = mr.."0"
	end
	return mr
end

function MessageFromBin(m)
	m = tostring(m)
	local msg = ""
	for i=1,m:len(),4 do
		local current = m:sub(i,i+3)
...
function RSAEncrypt(message, shared, publicKey, keylenght)
	print("Started RSA Encrypt [" .. os.date() .."]")
	print("Key length: " .. keylenght)
	local bin_m = MessageToBin(message, keylenght)
	local bi = bigint(bin_m)
	modPow(bi, bigint(publicKey), bigint(shared))
end

if not file_exists("public.key") or not file_exists("private.key") then
	...
	local f = io.open("public.key", "wb")
	f:write("return "..table.serialize(publicKey))
	f:close()
	f = io.open("private.key", "wb")
	f:write("return "..table.serialize(privateKey))
	f:close()
...
f:close()
local publicKey = load(fcontent)()

RSAEncrypt("Hello World", publicKey["shared"],publicKey["public"],39)
