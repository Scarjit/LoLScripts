...
file2 = "/S1mpleScripts/images/Veigar_Ziggs.png"
name = "test.lua"
name2 = "test.png"
path = SCRIPT_PATH
...
--[[
	...
	for i=1,10 do
		DL:newDL("www.xivia.website", "/scripts/xawareness/sprites/Hero_round/Aatrox.png", "Aatrox"..i..".png", path, function ()
			print("QQQ")
		end)
	end
...
function Download:__init()
	...
	function Async:tick()
		...
		elseif self.DLcStatus and self.DLcStatus == "timeout" then
			Download:Log("timed out")
			Download:Log("Data: \n")
			Download:Log(self.DLdata)
			Download:Log("\n====================================\n")
			self.DLsocket:close()
...
function Download:RemoveDone()
	...
	for k, v in pairs(self.aktivedownloads) do
		if v.progress < 100 then
			...
			self:Log("Triggerd Callback")
			self.callbacks[k]()
			self:Log("#self.aktivedownloads: "..#self.aktivedownloads)
		end
	end
	self.aktivedownloads = {}
...
function Download:Log(msg)
	if msg == nil then
		WriteFile("nil\n", SCRIPT_PATH.."\\AsyncLog.log","a")
	else
		WriteFile(tostring(msg).."\n", SCRIPT_PATH.."\\AsyncLog.log","a")
	end
end
