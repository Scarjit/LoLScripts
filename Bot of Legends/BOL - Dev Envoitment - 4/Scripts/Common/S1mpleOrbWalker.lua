...
--[[
S1mpleOrbWalker API Version 3.2
	...
	Methods:
		...
		:GetTarget(range) - returns the Current OrbWalker Target / or the best target in range
...
function S1mpleOrbWalker:GetTarget(r)

	local range = r and r or self.aarange
	local target = nil
	local mode = self:GetOrbMode()
	if mode == "none" then return end
	if self.forcedTarget.time > os.clock() then
		if ValidTarget(self.forcedTarget.target) and GetDistance(target) <= range then
	...
	if mode == "Combo" then
		target = TargetSelector:GetEnemyHero(range)
	elseif mode == "Harass" then
		target = TargetSelector:GetEnemyHero(range)
		if not target then target = TargetSelector:GetEnemyMinion(range) end
	...
	elseif mode == "Laneclear" then
		if not self.menu.misc.preferm then
			target = TargetSelector:GetStructurTarget(range)
		end
		if not target then
			target = TargetSelector:GetEnemyMinion(range)
		end
		if self.menu.minion.jungle and not target then
			target = TargetSelector:GetJungleMinion(range)
		end
		...
		if target and target.type == "obj_AI_Minion" then
			...
			if not self:CanLastHit(target) then
				local minions = TargetSelector:GetAllEnemyMinions(range)
...
function Download:__init()
	...
	function Async:tick()
		...
		elseif self.HEADcStatus == "timeout" or self.HEADcStatus == "closed" then
		...
		if self.HEADcStatus == "closed" and self.StartedDownload == true and self.canDL == true then --Double Check
			...
			--Start Main Download
			self.DLsocket:send('GET '..self.filepath..' HTTP/1.0'.. self.CRLF ..'Host: '..self.host.. self.CRLF ..'User-Agent: Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36'.. self.CRLF .. self.CRLF)
		end
		...
		elseif self.DLcStatus and self.DLcStatus == "timeout" or self.DLcStatus == "closed" then
			self.DLsocket:close()
