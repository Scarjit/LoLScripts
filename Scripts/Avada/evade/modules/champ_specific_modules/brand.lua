...
brand = common.Class(function (s, hero)
  ...
  print("Loaded brand submodule")
end)

function brand:IsValidSpell (spell)
  ...
  if(spell.name == "BrandW")then return true end
  return false
end

function brand:recvspell (spell)
  if(not self:IsValidSpell(spell)) then return end
  local timeout = os.clock() + spell.windUpTime
  self.spells[#self.spells+1] = {timeout = timeout, time = os.clock(), width = 260, endPos = vec3(spell.endPos.x,spell.endPos.y,spell.endPos.z)}
end

function brand:deletespell ()
