...
diana = common.Class(function (s, hero)
  ...
  print("Loaded diana submodule")
end)

function diana:IsValidSpell (spell)
  ...
  if(spell.name == "DianaArc")then return true end
  return false
end

function diana:recvspell (spell)
  if(not self:IsValidSpell(spell)) then return end
  local timeout = os.clock() + spell.windUpTime
  self.spells[#self.spells+1] = {timeout = timeout, time = os.clock(), width = 195, endPos = vec3(spell.endPos.x,spell.endPos.y,spell.endPos.z)}
end

function diana:deletespell ()
