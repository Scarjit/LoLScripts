...
bard = common.Class(function (s, hero)
  ...
  print("Loaded bard submodule")
end)

function bard:IsValidSpell (spell)
  ...
  if(spell.name == "BardR")then return true end
  return false
end

function bard:recvspell (spell)
  if(not self:IsValidSpell(spell)) then return end
  local timeout = os.clock() + spell.windUpTime
  
  if(spell.name == "BardR")then
