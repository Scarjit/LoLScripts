...
host = "raw.github.com"
file = "/Mr-Skizo/Skizophrenic/master/SkizophrenicLeBlanc.lua"
name = "test2.lua"
version = 0.1
versionfilepath = "/Mr-Skizo/Skizophrenic/master/Nidalee.version.txt"
...
function Download:__init()
	...
	function Async:tick()
		...
		if self.HEADcStatus == "closed" and self.StartedDownload == true and self.canDL == true then --Double Check
			...
			--Start Main Download
			self.DLsocket:send('GET '..self.filepath..' HTTP/1.1'.. self.CRLF ..'Host: '..string.sub(self.host,-1).. self.CRLF ..'User-Agent: Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36'.. self.CRLF .. self.CRLF)
		end
		...
		elseif self.DLcStatus and self.DLcStatus == "timeout" then
			...
			if self.isGitHub then
				--self.DLdata = Base64Decode(self.DLdata)
			end
	...
	function Async:draw()
		DrawTextA('HEAD '..self.filepath..' HTTP/1.1'.. self.CRLF ..'Host: '..self.host.. self.CRLF ..'User-Agent: Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36'.. self.CRLF .. self.CRLF)
		DrawTextA(self.DLdata,12,20,80)
