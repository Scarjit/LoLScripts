...
function FeezLeona:LoadProperties()
	...
	DP:bindSS("E", self.DivineSpells[_E], 50)
	DP:bindSS("R", self.DivineSpells[_R], 50)
...
function FeezLeona:Combo()
	...
	local DistanceToTarget = GetDistanceSqr(self.ts.target)

	if not self.ts.target.canMove then		
		if self.Spells[_W].Ready() and DistanceToTarget < self.Spells[_W].Range ^ 2 then
			CastSpell(_W)
		end
		if self.Spells[_Q].Ready() and DistanceToTarget < self.Spells[_Q].Range ^ 2 then
			CastSpell(_Q)
		end
	else
		if self.Spells[_Q].Ready() and DistanceToTarget < self.Spells[_Q].Range ^ 2 then
...
function FeezLeona:CastE()
	...
	if self.Config.misc.prediction == 1 then
		...
		local Status
		Status, CastPos, HitChance = DP:predict("E", self.ts.target)
...
function FeezLeona:CastR()
	...
	if ValidTarget(self.ts.target, self.Spells[_R].Range) and (VP.TargetsImmobile[self.ts.target.networkID] and VP.TargetsImmobile[self.ts.target.networkID] > os.clock() + .75) then
		...
		if self.Config.misc.prediction == 1 then
			...
			CastPos, HitChance = DP:predict("R", self.ts.target)
			HitChance = HitChance / 25
		end
...
function FeezLeona:OnProcessSpell(unit, spell)
	...
	if unit and unit.isMe and spell.target and spell.target.type == myHero.type and spell.name:lower():find("attack") then
		CastSpell(_Q)
	end
end
...
function FeezLeona:OnDraw()
	if self.Config.drawsettings.enabledraw then
		...
		if self.Config.drawsettings.r then
			self:DrawCircle(myHero, self.Spells[_R].Range, ARGB(255,36,0,255))
		end
		if self.Config.drawsettings.tstarget and self.ts.target then
