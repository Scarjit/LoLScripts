...
function Prediction:SetupSpells()
	...
	self.Q = { name = "SyndraQ", speed = math.huge, delay = 0.67, range = 790, width = 125, collision = false, aoe = true, type = "circular", dmgAP = function(source, target) return ((source:GetSpellData(_Q).level == 5 and target.type == myHero.type) and 1.15 or 1)*(5+45*source:GetSpellData(_Q).level+0.6*source.ap) end}
	...
	AddCreateObjCallback(function (object)
		if object and object.valid and object.name and object.name == "Seed" then
			if self.sspheres > 0 then
				self.sphere[#self.sphere+1] = {obj = object, endT = os.clock()+6.0-GetLatency()/2000-(SphereManager.stimeadd/SphereManager.sspheres), grabbed = false, netID = object.networkID}
			else
				self.sphere[#self.sphere+1] = {obj = object, endT = os.clock()+6.0-GetLatency()/2000, grabbed = false, netID = object.networkID}
			end
		end
	end)
...
function Syndra:combo()
	...
	if menu.ls.r and Prediction.R.dmgAP(myHero, target) > t.health*1.1 then
		SpellR:Cast(t)
	end
end
...
function Menu()
	...
	menu:addSubMenu("Spell Settings", "spells")
		...
		menu.spells:addSubMenu("W", "w")
			...
			menu.spells.w:addParam("auto2ndW", "Auto 2nd W Cast", SCRIPT_PARAM_ONOFF, true)
