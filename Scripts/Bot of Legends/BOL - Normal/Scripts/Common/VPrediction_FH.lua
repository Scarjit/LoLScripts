...
function VPrediction:__init()
	...
	if not _G.VPredictionMenu then
		_G.VPredictionMenu = scriptConfig("VPrediction+", "VPrediction___FH")
			_G.VPredictionMenu:addParam("Mode", "Cast Mode", SCRIPT_PARAM_LIST, 1, {"Fast", "Slow" })
...
function VPrediction:OnApplyBuff(source, unit, buff)
	if not unit or not buff or unit.type ~= myHero.type or _G.FH_Vpred_buffs == false then return end
