...
function Riven:AutoUpdater()
  local host = "www.s1mplescripts.de"
...
function Riven:Combo2(target)
  if ValidTarget(target) and self:Keys() == "Combo" then
    if self.Eready and self.Rready and self.Wready and self.Qready then -- noch Check hinzufÃ?Æ?Ã?â??Ã?â??Ã?Â¼gen fÃ?Æ?Ã?â??Ã?â??Ã?Â¼r Flash, Menu Flash nutzen
      self:CastE(target)
      self:CastR(target)
...
function Riven:LoadOrb()
  ...
  elseif self.OrbWalkers[Menu.Orbwalkers.Orbwalker] == "S1mpleOrbWalker" then 
    ...
    DelayAction(function()
        ...
        _G.S1:AddToMenu(Menu.Orbwalkers)
      end, 1)
  end
end
...
function Riven:Keys()
  ...
  if _G.S1:GetOrbMode() ~= "none" then
    return _G.S1:GetOrbMode()
  end
  end
end
