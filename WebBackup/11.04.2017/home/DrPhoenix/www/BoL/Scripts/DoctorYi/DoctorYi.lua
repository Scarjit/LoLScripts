...
Changelog :
	...
	Official release :
		...
		2.01 : Doctor Yi 2.0 is here boys !
                  ...
                  - More FPS
		2.02 : Add options to disable spells in clear modes
...
local function DrawMenu()
	...
	Menu:addSubMenu("JungleClear", "JungleClear")
		Menu.JungleClear:addParam("UseQ", "Use Alpha Strike (Q) in jungle clear", SCRIPT_PARAM_ONOFF, true)
		Menu.JungleClear:addParam("UseE", "Use Wuju Style (E) in jungle clear", SCRIPT_PARAM_ONOFF, true)

	Menu:addSubMenu("LaneClear", "LaneClear")
		Menu.LaneClear:addParam("UseQ", "Use Alpha Strike (Q) in lane clear", SCRIPT_PARAM_ONOFF, true)
...
local function Combo()
	if target and target.visible and target.valid and not target.dead then
		if myHero:CanUseSpell(_R) == READY and GetDistance(target) <= 2000 and Menu.Combo.UseRCombo then
			CastSpell(_R)
		end
...
local function Clear()
	for i, minion in pairs(jungleMinions.objects) do
		if i == 1 and minion and minion.valid and not minion.dead and minion.charName ~= "SRU_CampRespawnMarker" then
			if myHero:CanUseSpell(_E) and Menu.JungleClear.UseE then
				CastSpell(_E)
			end
			if myHero:CanUseSpell(_Q) and Menu.JungleClear.UseQ then
				CastSpell(_Q, minion)
			end
		end
	end
	for i, minion in pairs(minions.objects) do
		if i == 1 and minion and minion.valid and not minion.dead then
			if myHero:CanUseSpell(_Q) and Menu.LaneClear.UseQ then
				CastSpell(_Q, minion)
			end
		end
	end
