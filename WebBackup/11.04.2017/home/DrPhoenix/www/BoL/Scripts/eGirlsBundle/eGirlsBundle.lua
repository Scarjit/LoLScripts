...
function _Janna:AddMenu()
		self.Menu.KeySettings:addParam("AutoHealON", "Auto Heal", SCRIPT_PARAM_ONKEYTOGGLE, true, string.byte("H"))

		self.Menu.Combo:addParam("ComboQ", "Use Howling Gale (Q)", SCRIPT_PARAM_ONOFF, true)
		self.Menu.Combo:addParam("ComboW", "Use Zephyr (W)", SCRIPT_PARAM_ONOFF, true)
		self.Menu.Combo:addParam("ComboR", "Use Monsoon (R)", SCRIPT_PARAM_ONOFF, true)
		
		self.Menu:addSubMenu("Auto Heal", "AutoHeal")
			self.Menu.AutoHeal:addParam("AutoHealHealth", "AutoHeal if health < (%)", SCRIPT_PARAM_SLICE, 50, 0, 100, 0)
			self.Menu.AutoHeal:addParam("AutoHealMana", "AutoHeal if mana > (%)", SCRIPT_PARAM_SLICE, 20, 0, 100, 0)
		
		self.Menu.MiscSettings:addParam("SetSkin", "Select Skin:", SCRIPT_PARAM_LIST, 1, {"Classic", "Tempest", "Hextech", "Frost Queen", "Victorious", "Forecast", "Fnatic"})
		...
		self.Menu.DrawSettings:addParam("DrawQColor", "Q color", SCRIPT_PARAM_COLOR, {255, 129, 212, 226})
		self.Menu.DrawSettings:addParam("DrawW", "Draw W range", SCRIPT_PARAM_ONOFF, true)
		self.Menu.DrawSettings:addParam("DrawWColor", "W color", SCRIPT_PARAM_COLOR, {255, 129, 226, 191})
		self.Menu.DrawSettings:addParam("DrawE", "Draw E range", SCRIPT_PARAM_ONOFF, true)
		self.Menu.DrawSettings:addParam("DrawEColor", "E color", SCRIPT_PARAM_COLOR, {255, 129, 165, 226})
		self.Menu.DrawSettings:addParam("DrawR", "Draw R range", SCRIPT_PARAM_ONOFF, true)
		self.Menu.DrawSettings:addParam("DrawRColor", "R color", SCRIPT_PARAM_COLOR, {255, 140, 226, 129})
end

function _Janna:SetupUPL()
	UPL:AddToMenu(self.Menu)
	UPL:AddSpell(_Q, {speed = 900, delay = 0.3, range = 850, width = 120, collision = false, aoe = true, type = "linear"})
end

function _Janna:tick()
	if self.Menu.KeySettings.ComboON then
		self:Combo()
	end
	
	if self.Menu.KeySettings.AutoShieldON and not self.Menu.KeySettings.ComboON and not IsRecalling then
		self:AutoHeal()
	end
...
function _Janna:Animation(unit, animation)
    if unit.isMe then
...
function _Janna:AutoHeal()
	if (myHero.health / myHero.maxHealth * 100) < self.Menu.AutoHeal.AutoHealHealth and ( ( myHero.health / myHero.maxHealth * 100) < 10 or (myHero.mana / myHero.maxMana * 100) > self.Menu.AutoHeal.AutoHealMana ) then
		CastSpell(_W, myHero)
	else
		for i, hero in pairs(GetAllyHeroes()) do
			if hero and not hero.dead and GetDistance(myHero, hero) < 725 and (hero.health / hero.maxHealth * 100) < self.Menu.AutoHeal.AutoHealHealth and ( ( hero.health / hero.maxHealth * 100) < 10 or (myHero.mana / myHero.maxMana * 100) > self.Menu.AutoHeal.AutoHealMana ) then
				CastSpell(_W, hero)
			end
		end
	end
...
function _Nami:AddMenu()
		...
		self.Menu.MiscSettings:addParam("SetSkin", "Select Skin:", SCRIPT_PARAM_LIST, 1, {"Classic", "Koi", "River Spirit", "Urf", "Chroma Pack: Sunbeam", "Chroma Pack: Smoke", "Chroma Pack: Twilight"})
			self.Menu.MiscSettings:setCallback("SetSkin", function (value) SetSkin(myHero, self.Menu.MiscSettings.SetSkin - 1) end)
...
function _Sona:AddMenu()
		...
		self.Menu.MiscSettings:addParam("SetSkin", "Select Skin:", SCRIPT_PARAM_LIST, 1, {"Classic", "Muse", "Pentakill", "Silent Night", "Guqin", "Arcade", "DJ"})
			self.Menu.MiscSettings:setCallback("SetSkin", function (value) SetSkin(myHero, self.Menu.MiscSettings.SetSkin - 1) end)
...
function _Sona:AutoHeal()
	if (myHero.health / myHero.maxHealth * 100) < self.Menu.AutoHeal.AutoHealHealth and ( ( myHero.health / myHero.maxHealth * 100) < 10 or (myHero.mana / myHero.maxMana * 100) > self.Menu.AutoHeal.AutoHealMana ) then
		...
		for i, hero in pairs(GetAllyHeroes()) do
			if hero and not hero.dead and GetDistance(myHero, hero) < 1000 and (hero.health / hero.maxHealth * 100) < self.Menu.AutoHeal.AutoHealHealth and ( ( hero.health / hero.maxHealth * 100) < 10 or (myHero.mana / myHero.maxMana * 100) > self.Menu.AutoHeal.AutoHealMana ) then
				CastSpell(_W)
			end
		end
	end
...
function _Soraka:__init(Menu)
	...
	for _, hero in pairs(GetAllyHeroes()) do
		AllyUltCancel[hero.charName] = 0
	end
	GoingToCastUlt = false
	CancelUltCast = false
	ChampionToSave = nil
	...
	AddMsgCallback(function(msg, key)
		if not msg and not key then
			return
		else
			self:CancelUlt(msg, key)
		end
	end)
end
...
function _Soraka:AddMenu()
		...
		self.Menu:addSubMenu("Auto Heal", "AutoHeal")
			self.Menu.AutoHeal:addParam("WAutoHeal", "> AutoHeal with W",SCRIPT_PARAM_INFO, "")
			self.Menu.AutoHeal:addParam("WAutoHealHealth", "AutoHeal if health < (%)", SCRIPT_PARAM_SLICE, 50, 0, 100, 0)
			self.Menu.AutoHeal:addParam("WAutoHealMana", "AutoHeal if mana > (%)", SCRIPT_PARAM_SLICE, 20, 0, 100, 0)
			self.Menu.AutoHeal:addParam("WAutoHealLife", "AutoHeal if my health > (%)", SCRIPT_PARAM_SLICE, 20, 0, 100, 0)
			self.Menu.AutoHeal:addParam("AutoHealSpace", "",SCRIPT_PARAM_INFO, "")
			self.Menu.AutoHeal:addParam("RAutoHeal", "> AutoHeal with R",SCRIPT_PARAM_INFO, "")
			self.Menu.AutoHeal:addParam("RAutoHealHealth", "AutoHeal if health < (%)", SCRIPT_PARAM_SLICE, 10, 0, 100, 0)
			self.Menu.AutoHeal:addParam("RAutoHealMana", "AutoHeal if mana > (%)", SCRIPT_PARAM_SLICE, 20, 0, 100, 0)
		
		self.Menu.MiscSettings:addParam("SetSkin", "Select Skin:", SCRIPT_PARAM_LIST, 1, {"Classic", "Dryad", "Divine", "Celestine", "Reaper", "Order of the Banana"})
			self.Menu.MiscSettings:setCallback("SetSkin", function (value) SetSkin(myHero, self.Menu.MiscSettings.SetSkin - 1) end)
...
function _Soraka:tick()
	...
	if GoingToCastUlt and ( not ChampionToSave or not ChampionToSave.valid or ChampionToSave.dead ) then
		GoingToCastUlt = false
		ChampionToSave = nil
	end
end
...
function _Soraka:draw()
	...
	if GoingToCastUlt and ChampionToSave and AllyUltCancel[ChampionToSave.charName] < GetGameTimer() then
		DrawRectangle((WINDOW_W / 2) - 204, ((WINDOW_H / 5) * 3) - 4, 408, 158, ARGB(150,255,0,255))
		DrawRectangle((WINDOW_W / 2) - 200, (WINDOW_H / 5) * 3, 400, 150, ARGB(150,150,150,150))
		DrawTextA("Going to cast ult to save "..ChampionToSave.charName.." ("..tostring(math.floor(ChampionToSave.health/ChampionToSave.maxHealth*100)).."%),", 22, (WINDOW_W / 2), (WINDOW_H / 5) * 3 + 25, ARGB(255,255,0,255), "center", "center")
		DrawTextA("Would you like to cancel it ?", 30, (WINDOW_W / 2), (WINDOW_H / 5) * 3 + 55, ARGB(255,255,0,255), "center", "center")
		DrawRectangle((WINDOW_W / 2) - 104, ((WINDOW_H / 5) * 3) + 80, 208, 58, ARGB(255,255,0,255))
		DrawRectangle((WINDOW_W / 2) - 100, (WINDOW_H / 5) * 3 + 84, 200, 50, ARGB(255,200,0,200))
		DrawTextA(">> YES <<", 45, (WINDOW_W / 2), (WINDOW_H / 5) * 3 + 109, ARGB(255,255,0,255), "center", "center")
	end
end
...
function _Soraka:AutoHeal()
	for i, hero in pairs(GetAllyHeroes()) do
		if hero and not hero.dead and GetDistance(myHero, hero) < 550 and myHero:CanUseSpell(_W) == READY and self.Menu.AutoHeal.WAutoHealLife < (myHero.health / myHero.maxHealth * 100) and (hero.health / hero.maxHealth * 100) < self.Menu.AutoHeal.WAutoHealHealth and ( ( hero.health / hero.maxHealth * 100) < 10 or (myHero.mana / myHero.maxMana * 100) > self.Menu.AutoHeal.WAutoHealMana ) then
			CastSpell(_W, hero)
		end
		
		if hero and not hero.dead and myHero:CanUseSpell(_R) == READY and ( (hero.health / hero.maxHealth * 100) < self.Menu.AutoHeal.RAutoHealHealth or (myHero.health / myHero.maxHealth * 100) < self.Menu.AutoHeal.RAutoHealHealth ) and ( ( hero.health / hero.maxHealth * 100) < 10 or (myHero.mana / myHero.maxMana * 100) > self.Menu.AutoHeal.RAutoHealMana ) then
			GoingToCastUlt = true
			ChampionToSave = hero
			DelayAction(function()
				if not CancelUltCast and AllyUltCancel[hero.charName] < GetGameTimer() then
					GoingToCastUlt = false
					if hero and not hero.dead and myHero:CanUseSpell(_R) == READY then
						CastSpell(_R)
					end
				end
			end, 6)
...
function _Soraka:CancelUlt(msg, key)
	if key == 1 and msg == 513 then
		if GetCursorPos().x > (WINDOW_W / 2) - 104 and GetCursorPos().x < (WINDOW_W / 2) + 104 then
			if GetCursorPos().y > ((WINDOW_H / 5) * 3) + 80 and GetCursorPos().y < ((WINDOW_H / 5) * 3) + 138 then
				AllyUltCancel[ChampionToSave.charName] = GetGameTimer() + 15
				ChampionToSave = nil
				CancelUlt = false
				GoingToCastUlt = false
			end
