<?php
	function random_str($length = 32, $keyspace = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ')
	{
		$str = '';
	...
	//Validate HMAC
	$hmac = $split[2];
	$hmac_key = "390C38C4276911A3C625C288012B445CA11C9026B";
	...
	$iv = $split[0];
	$ciphertext = $split[1];
	$key = "9B427CB2D140096AB238F346AE27ECE6";
	...
	try{
		...
		if(sizeof($sql_get_is_hwid_banned->fetchAll()) > 0){
			...
			if(!$is_beta){
				...
				if($update_count == true){
					$sql_update_loads = $conn->prepare("UPDATE auths SET script_loads = script_loads+1 WHERE user_id = :uid");
					$sql_update_loads->bindParam(':uid', $user_id);
					$sql_update_loads->execute();
				}
		...
		//Create Session Key and transmit to client if authed
		$rand_key = random_str();
		if (preg_match('/(AUTHED)|(TRIAL)/',$return_message)) {
			if(!preg_match('/(TRIALEX)/',$return_message)){
				//Create session
				$sql_create_session_key = $conn->prepare("INSERT INTO session_keys (keyid, uid, sid, session_key) VALUES (NULL, :uid, :sid, :skey)");
				$sql_create_session_key->bindParam(':uid', $x_user_id);
				$sql_create_session_key->bindParam(':sid', $x_script_id);
				$sql_create_session_key->bindParam(':skey', $rand_key);
				$sql_create_session_key->execute();
			}	
		}
		
	    $return_message .= "||".$m_username."||";
		$return_message .= $rand_key."||";
