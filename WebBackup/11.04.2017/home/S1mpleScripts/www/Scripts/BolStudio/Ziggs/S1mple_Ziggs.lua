...
local version = "4.1"
local lolversion = "5.24"
local Update_HOST = "www.s1mplescripts.de"
local Update_PATH = "/S1mple/Scripts/BolStudio/Ziggs/S1mple_Ziggs.lua"
...
function Update(arg)
	if arg ~= "force" then
		...
		local ServerData = GetWebResult(Update_HOST, "/S1mpleScripts/Scripts/BolStudio/Ziggs/S1mple_Ziggs.version")
		if ServerData then
...
function ChkUpdate()
	...
	p("Checking for Updates")
			local ServerData = GetWebResult(Update_HOST, "/S1mple/Scripts/master/S1mple_Ziggs.version")
		if ServerData then
			ServerVersion = type(tonumber(ServerData)) == "number" and tonumber(ServerData) or nil
			if ServerVersion then
				if tonumber(version) < ServerVersion then
